...
def main(expt_name,
  ...
  for _ in range(num_repeats):
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session(config=tf_config) as sess:
      ...
      if val_loss < best_loss:
        ...
        print('Migliore epoca:', best_epoch)
  ...
  with tf.Graph().as_default(), tf.compat.v1.Session(config=tf_config) as sess:
    ...
    utils.save_to_directory_csv(p10_forecast, os.path.join(config.model_folder, "preds"), "test_predictions_p10.csv")
    utils.save_to_directory_csv(p50_forecast, os.path.join(config.model_folder, "preds"), "test_predictions_p50.csv")
    utils.save_to_directory_csv(p90_forecast, os.path.join(config.model_folder, "preds"), "test_predictions_p90.csv")
    utils.save_to_directory_csv(targets, os.path.join(config.model_folder, "targets"), "targets.csv")
    print('Prediction results saved')
    print('Start Attention Weights saving')
    get_attention_weights= model.get_attention(test)
    utils.save_to_directory_pkl(get_attention_weights, os.path.join(config.model_folder, "weights"), "weights.pkl")
    print('Ending Attention Weights saving')
