...
def build_model_ann(hp):
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Dense(hp.Choice('units1dense', [32, 64, 128, 256]),
                              activation=hp.Choice('activation1', ['relu', 'tanh'])),
        tf.keras.layers.Dropout(hp.Choice('dropout1', [0.25,0.5])),
        # Dense layer with regularization
        tf.keras.layers.Dense(hp.Choice('units2dense', [32, 64, 128,256]),
                              activation=hp.Choice('activation2', ['relu', 'tanh'])),
        tf.keras.layers.Dropout(hp.Choice('dropout2', [0.25, 0.5])),
        # Un altro layer Dense
        tf.keras.layers.Dense(hp.Choice('units3dense', [8, 16, 32, 64]),
                              activation=hp.Choice('activation3', ['relu', 'tanh']),
                              kernel_regularizer=regularizers.l2(0.00001)),
        # Un altro layer Dense
        tf.keras.layers.Dense(hp.Choice('units4dense', [8, 16, 32, 64]),
                              activation=hp.Choice('activation4', ['relu', 'tanh']),
                              kernel_regularizer=regularizers.l2(0.00001)),
        tf.keras.layers.Dropout(hp.Choice('dropout3', [0.25, 0.5])),
...
def defining_tuning(ticker,data,window_size,target_column,forecast_horizon):
    ...
    tuner = Hyperband(
        build_model_ann,
        objective='val_loss',
        max_epochs=50,  # Sostituisci con il numero massimo di epoche desiderato
        factor=3, # Valore tipico per il fattore è 3
        directory=f'./TUNER/tuner_ann_hv/{ticker}',
        project_name='ann-tuner-hv',
        overwrite=True
    )
    ...
    # Definisci il percorso del file JSON dove vuoi salvare i dati
        json_file_path = f"./hp/ann/best_hyperparameters_{ticker}.json"
    # Scrivi i dati nel file JSON
        ...
        print(f"Si è verificato un errore nel salvataggio hyperparametri: {e}")

    hp = HyperParameters()
    list_names = ['units1dense',
                  'activation1',
                  'dropout1',
                  'units2dense',
                  'activation2',
    ...
    print('Prediction y_test:', y_test.shape)
    print('Prediction X_train:', X_train.shape)
    print('Prediction y_train:', y_train.shape)
    print('Prediction X_val:', X_val.shape)
    print('Prediction y_val:', y_val.shape)
    ...
    try:
        results_df.to_csv(f'./TUNER/predictions/ann/{ticker}.csv', index=True)
        results_path= f'./TUNER/predictions/ann/{ticker}.csv'
        verifica_salvataggio_file(results_path)
    except Exception as e:
        print(f"Si è verificato un errore nel salvataggio delle predizioni : {e}")

    logging.info("Inizio del processo di pulizia delle cartelle")
    try:
        if verifica_esistenza_cartella(f'./TUNER/tuner_ann_hv/{ticker}'):
            # Operazioni sulla cartella, ad esempio eliminarla
            shutil.rmtree(f'./TUNER/tuner_ann_hv/{ticker}')
    except Exception as e:
        print(f"Si è verificato un errore nell'eliminazione della cartella: {e}")
